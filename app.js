...
function hideLoader(delay=0){
  const run=()=>document.getElementById("loader")?.classList.add("hide");
  if(delay>0) setTimeout(run,delay); else run();
}

function bootstrapStaticUi(){
  try{ applyAppearance(); }catch(e){ console.warn("applyAppearance failed",e); }
  try{ renderServices(); }catch(e){ console.warn("renderServices failed",e); }
  try{ renderManagedTicker(); }catch(e){ console.warn("renderManagedTicker failed",e); }
  try{ etqanSyncAdminUi(); }catch(e){ console.warn("etqanSyncAdminUi failed",e); }
  try{ etqanFillSettingsForm(); }catch(e){ console.warn("etqanFillSettingsForm failed",e); }
  try{ etqanRenderCmsContent(); }catch(e){ console.warn("etqanRenderCmsContent failed",e); }
  try{ initMemberPortal(); }catch(e){ console.warn("initMemberPortal failed",e); }
}
...
(async()=>{
  try{
    ...
    try{
      ...
      toast("تعذر تهيئة Firebase، تم تشغيل الواجهة بالوضع الاحتياطي");
    }

    if(db){
      try{ await loadSettings(); }catch(e){ console.warn("loadSettings failed",e); }
      try{ await loadServices(); }catch(e){ console.warn("loadServices failed",e); }
      try{ listenOrders(); }catch(e){ console.warn("listenOrders failed",e); }
      try{ listenReviews(); }catch(e){ console.warn("listenReviews failed",e); }
      try{ listenMembers(); }catch(e){ console.warn("listenMembers failed",e); }
      try{ listenGlobalMessages(); }catch(e){ console.warn("listenGlobalMessages failed",e); }
      try{ listenChatMetas(); }catch(e){ console.warn("listenChatMetas failed",e); }
      try{ listenNotifications(); }catch(e){ console.warn("listenNotifications failed",e); }
      try{ initAdminChatUi(); }catch(e){ console.warn("initAdminChatUi failed",e); }
      try{ etqanEnsureAdminEnhancements(); }catch(e){ console.warn("etqanEnsureAdminEnhancements failed",e); }
      try{ etqanRestoreAdminSession(); }catch(e){ console.warn("etqanRestoreAdminSession failed",e); }
    }
    ...
    try{ etqanBuildAccountRedesign(); }catch(e){ console.warn("etqanBuildAccountRedesign failed",e); }
  }catch(e){
    console.error(e);
    toast("حدث خطأ أثناء تشغيل المنصة، وتم عرض النسخة الاحتياطية");
  }finally{
    hideLoader(350);
  }
})();
...
window.addEventListener("load",()=>{ hideLoader(250); });
document.addEventListener("DOMContentLoaded",()=>{ setTimeout(()=>hideLoader(),2200); });
