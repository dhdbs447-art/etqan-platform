...
function initMemberPortal(){
 ...
 const authTabs=document.querySelector("#memberAuth .authTabs");
 if(authTabs) authTabs.classList.add("hidden");
 $("#memberLoginForm")?.classList.remove("hidden");
 $("#memberRegisterForm")?.classList.remove("hidden");
 $$("[data-member-mode]").forEach(btn=>btn.onclick=(e)=>{ e.preventDefault(); });
 $("#memberRegisterForm")?.addEventListener("submit",async e=>{
   ...
   currentMember={id:ref.id,...data}; localStorage.setItem("etqan_current_member",JSON.stringify(currentMember)); adminAuthenticated=false; etqanHideAdminCompletely();
 ...
 $("#memberLoginForm")?.addEventListener("submit",e=>{
   ...
   currentMember=member; localStorage.setItem("etqan_current_member",JSON.stringify(member)); adminAuthenticated=false; etqanHideAdminCompletely(); e.target.reset(); toast("تم دخول العضو بنجاح"); renderMemberDashboard();
 });
 $("#memberLogoutBtn")?.addEventListener("click",()=>{currentMember=null;localStorage.removeItem("etqan_current_member"); adminAuthenticated=false; etqanHideAdminCompletely(); stopMemberChat();renderMemberDashboard();toast("تم خروج العضو")});
 initMemberChatUi();
...
$("#loginBtn").onclick=()=>{
  ...
  if(user===String(settings.username||"").trim() && pass===String(settings.password||"")){
    ...
    adminAuthenticated=false; etqanHideAdminCompletely();
...
function etqanHideAdminCompletely(){
  ...
  $("#adminPanel")?.classList.add("hidden");
  $("#loginBox")?.classList.add("hidden");
  document.body.classList.remove("admin-authenticated","admin-mode");
}

function etqanOpenAdminLoginOnly(){
  adminAuthenticated=false;
  document.body.classList.remove("admin-authenticated");
  const adminSec=document.getElementById("admin");
...
function etqanSetAdminMode(on){
  const enabled=!!on && adminAuthenticated;
  document.body.classList.toggle("admin-mode",enabled);
  document.body.classList.toggle("admin-authenticated",enabled);
  const adminSec=document.getElementById("admin");
  if(adminSec){
    if(enabled) adminSec.classList.remove("roleHidden");
    else adminSec.classList.add("roleHidden");
  }
  if(!enabled){
    $("#adminPanel")?.classList.add("hidden");
  }
...
function initEtqanMobileAppNav(){
  ...
  document.getElementById("topSpecialistBtn")?.addEventListener("click",e=>{e.preventDefault(); etqanOpenAdminLoginOnly();});
