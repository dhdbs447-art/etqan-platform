...
try{ etqanBuildAccountRedesign(); }catch(e){} }catch(e){console.error(e);toast("تحقق من إعدادات Firebase والقواعد");etqanSafeHideLoader();}})();


// Elite Pro UI enhancements
function etqanHideLoader(reason="app-ready"){
  if(typeof window.etqanForceHideLoader === "function") return window.etqanForceHideLoader(reason);
  const loader=document.getElementById("loader");
  if(loader) loader.classList.add("hide");
}
window.addEventListener("load",()=>{
  setTimeout(()=>etqanHideLoader("window-load"),450);
});
